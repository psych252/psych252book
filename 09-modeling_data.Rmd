...
```{r modeling-04-likelihood1, fig.cap="Probability is the area under the curve of the density"}
...
```{r modeling-05-likelihood2, fig.cap="Likelihood is a particular value."}
point = 0
param_mean = 1
param_sd = 1
...
```{r modeling-06-likelihood3, fig.cap="Relationship between density and cumulative probability distribution."}
...
```{r modeling-07-likelihood4, fig.cap="The density is the first derivative of the cumulative probability distribution. The likelihood is the value of the slope in the cumulative probability distribution."}
...
```{r modeling-08-likelihood5, fig.cap="The relative likelihood of two observations is the same as the relative probability of two areas under the curve as the margin of these areas goes to 0."}
margin = 0.1
point_blue = -1
point_red = 0
...
```{r modeling-09, warning=F, fig.cap="Tradeoff between fit and model simplicity."}
...
```{r modeling-10, fig.cap="Figure that I used to illustrate that fitting more data points with fewer parameter is more impressive."}
# make example reproducible 
set.seed(1)
# n_samples = 20
n_samples = 3
...
```{r modeling-13, warning=FALSE, fig.cap="Sum of absolute errors."}
...
```{r modeling-15, echo=F, fig.cap="Relationship between error definition and best estimators."}
tibble(
...
```{r modeling-16, fig.cap="Mean, median, and mode on the normal distribution."}
...
```{r modeling-17, fig.cap="Mean, median, and mode on the exponential distribution."}
rate = 1 
...
```{r modeling-20, warning=F}
df.internet = read_table2(file = "data/internet_access.txt") %>% 
  clean_names()
```

```{r modeling-21, fig.cap="Selection of the data."}
df.internet %>% 
  mutate(i = 1:n()) %>% 
...
```{r modeling-22, fig.cap="Sum of squared errors, proportional reduction in error (PRE), F statistic, p-value."}
...
```{r modeling-23, fig.cap="The F distribution"}
df1 = 1
df2 = 49
