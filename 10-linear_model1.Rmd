...
```{r linear-model1-04, warning=F, message=F}
...
```{r linear-model1-05, warning=F, message=F}
samples %>% 
  as_tibble(.name_repair = "unique") %>% 
  mutate(sample = 1:n()) %>% 
  gather("index", "value", -sample) %>% 
...
```{r linear-model1-24, echo=F, fig.caption="Credit card data variable description."}
tibble(
...
- [Visualize residuals with the ggResidpanel package](https://cran.r-project.org/web/packages/ggResidpanel/vignettes/introduction.html)
- [Show equations for linear models](https://github.com/datalorax/equatiomatic)
