...
```{r linear-model01, message=FALSE}
...
```{r linear-model03, warning=F, message=F}
...
```{r linear-model04, warning=F, message=F}
samples %>% 
  as_tibble(.name_repair = "unique") %>% 
  mutate(sample = 1:n()) %>% 
  gather("index", "value", -sample) %>% 
...
```{r linear-model23, echo=F, fig.caption="Credit card data variable description."}
tibble(
