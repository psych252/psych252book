...
mytheme = list(scale_color_brewer(palette = "Set1"),
...
Let's try to better understand interactions in logistic regressions. First, let's review interactions in the linear model. We will focus on the case in which we have one binary predictor and one continuous predictor. 

Let's simulate and visualize some data with an interaction. Let's start with the linear regression. 
...
# simulated data 
df.interaction_linear = tibble(dummy = sample(0:1, size = n, replace = T),
                               continuous = runif(n = n),
                               y = b0 +
                                 b1 * dummy +
                                 b2 * continuous + 
                                 b3 * dummy * continuous + 
...
# visualize the data 
ggplot(data = df.interaction_linear,
       mapping = aes(x = continuous,
                     y = y,
                     color = dummy)) +
  geom_point() +
  geom_smooth(method = "lm",
              mapping = aes(fill = dummy)) + 
  mytheme

# fit the linear model 
fit = lm(formula = y ~ 1 + dummy * continuous,
         data = df.interaction_linear)

fit %>% 
  summary()
```

We can easily spot the interaction here since the two lines are intersecting one another. Now let's do the same thing for a logistic regression. The main trick is that we use the inverse-logit linking function to transform our dependent variable into probability space. 
...
fun_logistic_regression = function(betas){
  
df.interaction_logistic = tibble(dummy = sample(0:1, size = n, replace = T),
                               offset = runif(n = n),
                               y = betas[1] +
                                 betas[2] * dummy +
                                 betas[3] * offset + 
                                 betas[4] * dummy * offset) %>% 
  mutate(p = exp(y) / (1 + exp(y)),
         dummy = as.factor(dummy)) %>% 
  mutate(y = rbinom(n(), size = 1, p = p))

# visualize the results

p1 = ggplot(data = df.interaction_logistic,
       mapping = aes(x = offset,
                     y = y,
                     color = dummy,
                     fill = dummy)) +
  geom_point(alpha = 0.1,
...
# fit the glm 
fit = glm(formula = y ~ 1 + dummy * offset,
          family = "binomial",
          data = df.interaction_logistic)

# visualize predictions in log/odds space 
p2 = fit %>% 
  augment() %>% 
  clean_names() %>% 
  ggplot(data = .,
...
wrap_plots(c(fun_logistic_regression(betas1),
             fun_logistic_regression(betas2),
             fun_logistic_regression(betas3)),
           ncol = 2)
```

Neat! On the right side 
