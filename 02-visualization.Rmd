...
- mention the "overlapping problem"" when making plots with points and errorbars 
  + look at Natalia Velez's solution 
...
library("knitr")
library("waffle")
library("ggpol") # additional visualization functions
library("janitor")
library("gganimate")
library("gapminder")
library("tidyverse")
...
- [ggplot2 visualizations with code](http://r-statistics.co/Top50-Ggplot2-Visualizations-MasterList-R-Code.html)
...
ggplot(data = df.plot, aes(x = x, y = y, shape = shape))+
  geom_point(fill = "red", size = 5, show.legend = F)+
  geom_text(aes(label = shape), vjust = -1.3, size = 5)+
...
#### Hybrid boxplot and jitter 

```{r}
ggplot(data = diamonds %>% sample_n(1000), aes(x = color, y = price))+
...
ggplot(data = df.plot, aes(x = year, y = median, color = city, fill = city))+
  ...
  stat_summary(fun.y = "mean", geom = 'point')+
  stat_summary(fun.y = "mean", geom = 'line')+
  geom_text(data = df.text, aes(label = city), hjust = -0.1, size = 5)+
  ...
  theme(title = element_text(size = 16),
        legend.position = "none",
        plot.margin = margin(r = 1.2, unit = "inch")
        )
