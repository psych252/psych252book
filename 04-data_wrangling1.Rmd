...
Personally, I prefer to write down the function using the `~ function(.)` syntax to make it clear that I'm calling a function. Within `vars()`, we can use the same helper functions for selecting columns that we've seen above for `select()`. 
