...
```{r mediation-01, echo=FALSE, eval=FALSE, include=FALSE}
install.packages(c("mediation", "multilevel"))
```

```{r mediation-02, message=FALSE}
...
```{r mediation-04, echo=FALSE, out.width="75%", fig.cap="__Basic mediation model__. c = the total effect of X on Y; c = c’ + ab; c’ = the direct effect of X on Y after controlling for M; c’ = c - ab; ab = indirect effect of X on Y."}
...
df.samples %>% 
  select(ab, total) %>%
  pivot_longer(names_to = "effect", values_to = "value", cols = everything()) %>% 
...
```{r mediation-18, echo=FALSE, out.width="75%", fig.cap="__Basic moderation model__."}
