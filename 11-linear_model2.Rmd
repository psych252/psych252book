...
```{r linear-model2-10, fig.cap="Bar plot illustrating how strongly different variables correlate with income."}
df.credit %>% 
  select_if(is.numeric) %>%
  correlate(quiet = T) %>%
...
```{r linear-model2-11, fig.caption = "Correlation plot showing the pairwise correlations between different variables."}
tmp = df.credit %>%
  select_if(is.numeric) %>%
...
```{r linear-model2-12, fig.cap="Pairwise correlations with scatter plots, correlation values, and densities on the diagonal."}
df.ads %>%
  select(-index) %>% 
  ggpairs()
```
...
```{r linear-model2-13, fig.cap="Pairwise correlations with scatter plots, correlation values, and densities on the diagonal (customized)."}
df.ads %>% 
  select(-index) %>%
