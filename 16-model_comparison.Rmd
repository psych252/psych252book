...
```{r model-comparison-06, eval=FALSE}
...
df.cross = df.data %>% 
  ...
  pivot_longer(names_to = "model", values_to = "fit", cols = contains("model")) %>% 
...
- [purrr](figures/cheatsheets/purrr.pdf)
- [cross-validation for model selection](https://github.com/LudvigOlsen/cvms): cross-validation in multi-level designs
...
- [caret](http://topepo.github.io/caret/index.html): General framework for modeling data in R. 
