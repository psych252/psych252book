...
fit.brm1 %>% 
  ...
  mode_hdi() %>% 
  pivot_longer(names_to = "index", values_to = "value", cols = -c(.width:.interval)) %>% 
  select(index, value) %>% 
...
```{r bda3-12, message=FALSE, interval=1/120}
...
df.predictive_samples %>% 
  select(-data) %>% 
  pivot_longer(names_to = "hand", values_to = "balance", cols = -sample) %>% 
...
df.variance %>%
  ...
  pivot_longer(names_to = "index", values_to = "value", cols = c(mu, sigma)) %>% 
...
- [Tutorial on constrasts in Bayes](https://janhove.github.io/analysis/2018/12/20/contrasts-interactions)
