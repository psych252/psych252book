...
```{r lmer3-31, warning=F, message=F}
# complete pooling
...
df.plot = df.sleep %>% 
  ...
  pivot_longer(names_to = "index", values_to = "value", cols = -c(subject, method)) %>% 
...
```{r lmer3-42, message=F, warning=F}
# fit the model 
fit.lmer = lmer(formula = reaction ~ 1 + days + (1 + days | subject),
                data = df.sleep)
...
boot.lmer$t %>% 
  ...
  clean_names() %>% 
  mutate(id = 1:n()) %>% 
  pivot_longer(names_to = "index", values_to = "value", cols = - id) %>% 
  ggplot(data = .,
       mapping = aes(x = value)) + 
  geom_density() + 
  facet_grid(cols = vars(index),
