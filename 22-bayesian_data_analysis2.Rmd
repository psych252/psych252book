...
```{r bda2-06}
df.coins %>% 
  pivot_longer(names_to = "index", values_to = "value", cols = -theta) %>% 
...
```{r bda2-07, fig.cap="Illustration of how the prior affects the posterior."}
# grid
theta = seq(0, 1, 0.01)
...
df.prior_effect = tibble(theta = theta, 
  ...
  pivot_longer(names_to = "prior_index", values_to = "prior", cols = -theta) %>% 
  ...
  ungroup() %>% 
  pivot_longer(names_to = "index", values_to = "value", cols = -c(theta, prior_index))
...
```{r bda2-08, fig.cap="Illustration of how the likelihood of the data affects the posterior."}
# grid
theta = seq(0, 1, 0.01)
...
df.likelihood_effect = tibble(theta = theta, 
  ...
  pivot_longer(names_to = "likelihood_index", values_to = "likelihood", cols = -c("theta", "prior")) %>% 
  group_by(likelihood_index) %>% 
  mutate(posterior = likelihood * prior / sum(likelihood * prior)) %>% 
  ungroup() %>% 
  pivot_longer(names_to = "index", values_to = "value", cols = -c(theta, likelihood_index))
...
```{r bda2-10, fig.cap="Comparison between the normal distribution and the student-t distribution."}
tibble(x = c(-5, 5)) %>% 
  ggplot(aes(x = x)) + 
...
```{r bda2-11, fig.cap="Beta distributions with different parameter settings."}
...
```{r bda2-12, fig.cap="Normal distributions with different standard deviation."}
tibble(x = c(-10, 10)) %>% 
  ggplot(aes(x = x)) + 
  stat_function(fun = "dnorm",
                size = 1,
...
```{r bda2-13, fig.cap="Cauchy and Gamma distribution."}
tibble(x = c(0, 10)) %>% 
  ggplot(aes(x = x)) + 
...
```{r bda2-21, cache=TRUE, message=FALSE}
# sampling
...
df.draws %>% 
  select(draw:sd) %>% 
  pivot_longer(names_to = "index", values_to = "value", cols = -draw) %>% 
  ggplot(data = .,
         mapping = aes(x = value)) + 
  stat_density(geom = "line") + 
  facet_grid(rows = vars(index),
...
```{r bda2-25, message=FALSE, interval=1/60}
df.draws %>% 
  sample_n(size = 10) %>%  
...
```{r bda2-26, message=FALSE, interval=1/60}
sample_size = 10
